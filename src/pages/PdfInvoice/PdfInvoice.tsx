...
function PdfInvoice() {
  ...
  async function handleDeleteAutoGeneratedInvoice(invoice: any) {
    if (deletingAutoId) return;
    setDeletingAutoId(invoice.id);
    try {
      ...
      queryClient.setQueryData<any[]>(["auto-generated-pdf-invoices", coachId], (prev) =>
  ...
  return (
    <div className="pdfinv-page">
          ...
          <div className="pdfinv-body">
            {autoGeneratedLoading ? (
              ...
              autoGeneratedInvoices.map((invoice: any) => {
                ...
                return (
                  <section key={invoice.id} className="pdfinv-card pdfinv-auto-card">
                    <button
                      type="button"
                      className="pdfinv-auto-delete-btn"
                      disabled={deletingAutoId === invoice.id}
                      onClick={() => setInvoiceToDelete(invoice)}
                      aria-label="Delete invoice"
                      title="Delete invoice"
                    >
                      {deletingAutoId === invoice.id ? <FaSpinner className="spin" /> : <FaTrash />}
                    </button>
      ...
      {invoiceToDelete && (
        <div className="billio-confirm-overlay" onClick={() => setInvoiceToDelete(null)}>
          <div className="billio-confirm-card" onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              This will permanently delete invoice <strong>{invoiceToDelete.invoice_number}</strong>. This cannot be undone.
            </p>
            <div className="billio-confirm-actions">
              <button type="button" className="billio-cancel-btn" onClick={() => setInvoiceToDelete(null)}>
                Cancel
              </button>
              <button
                type="button"
                className="billio-danger-btn"
                disabled={deletingAutoId === invoiceToDelete.id}
                onClick={() => handleDeleteAutoGeneratedInvoice(invoiceToDelete)}
              >
                {deletingAutoId === invoiceToDelete.id ? "Deleting..." : "Delete"}
              </button>
            </div>
          </div>
        </div>
